...
export default function Test1() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const questions = await await getQuestions("verbs,adjetives,kitchen,city,room", 10, 4)
  ...
  return (
    <div className="container">
      <div className={`${(39 < level) | isAnswerCorrect ? "d-none" : ""}`}>
      ...
      <div className={`${!((39 < level)) ? "d-none" : ""} text-center`}>
        <h1>Felicitaciones ya ha completado el módulo 3 del nivel intermedio!</h1>
        <img
        ...
        <div className="m-auto text-center">
          <a href={`/lectures/intermediate/es/location`} className={`text-center btn btn-success ${(39 < level) | isAnswerCorrect ? "" : "d-none m-auto"}`}>Continuar</a>
        </div>
      </div>
    </div>
  );
